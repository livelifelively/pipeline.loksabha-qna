...
async def create_new_iteration(progress_file: Path) -> ProgressIteration:
    ...
    try:
        ...
        return ProgressIteration(
            iteration=len(progress_status) + 1,
...
async def log_progress(
    ...
    try:
        ...
        existing_index = next(
            (i for i, iter in enumerate(progress_status) 
             if iter["iteration"] == iteration.iteration),
            -1
        )
        ...
        raise ProgressError(f"Failed to log progress: {e}", {"original_error": str(e)})
